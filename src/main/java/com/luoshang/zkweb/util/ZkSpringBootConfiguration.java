...
public class ZkSpringBootConfiguration implements WebMvcConfigurer {
	...
	public DataSource getDataSource() {
		...
		ComboPooledDataSource dataSource = org.springframework.boot.jdbc.DataSourceBuilder.create()
		...
		dataSource.setIdleConnectionTestPeriod(
				Integer.parseInt(env.getProperty("spring.datasource.idle-connection-test-period")));
		...
		dataSource
				.setAcquireRetryAttempts(Integer.parseInt(env.getProperty("spring.datasource.acquire-retry-attempts")));
	...
	public void addInterceptors(InterceptorRegistry registry) {
		...
		// JoeInterceptor()).addPathPatterns("/freemarker/**");
		// addPathPatterns 用于添加拦截规则
		// excludePathPatterns 用户排除拦截
		// registry.addInterceptor(new
		// MyInterceptor()).addPathPatterns("/**").excludePathPatterns("/zk","/zkcfg");
	}
	...
	public RequestMappingHandlerAdapter requestMappingHandlerAdapter() {
		...
		// stringHttpMessageConverter.setSupportedMediaTypes(Lists.newArrayList(new
		// MediaType("text/html;charset=UTF-8"),
		// new MediaType("text/plain;charset=UTF-8")));
		// MappingJackson2HttpMessageConverter mappingJackson2HttpMessageConverter=new
		// MappingJackson2HttpMessageConverter();
		// mappingJackson2HttpMessageConverter.setSupportedMediaTypes(Lists.newArrayList(new
	...
	// return new ServletRegistrationBean<ZkCacheServlet>(new
	// ZkCacheServlet(),"/cache/*");
	// }
}
