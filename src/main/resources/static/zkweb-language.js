...
var i18nSettings = {
	...
	mode : 'map',
	language : $.i18n.normaliseLanguageCode({
		language : ""
	}), // $.i18n.browserLang(), //这就是国家代号 name+language刚好组成属性文件名：strings+zh ->
...
var getNavLanguage = function() {
	if (navigator.appName == "Netscape") {
		return $.i18n.normaliseLanguageCode({
			language : ""
		});
	}
	return "en";
...
var getLanguage = function() {
	var language = getLanguageFromCookie(getNavLanguage());
...
var setLanguage = function(node) {
	...
	if (cookielanguage && cookielanguage != language) {
		console.log('1.cookielanguage=' + cookielanguage + ',newlanguage='
				+ language)
		getCookie(i18nCookiename, language);
	} else if (cookielanguage && cookielanguage == language) {
		console.log('2.cookielanguage=' + cookielanguage + ',newlanguage='
				+ language)
	} else {
		console.log('3.cookie[' + i18nCookiename + ']=' + language)
		getCookie(i18nCookiename, language);
	}
	console.log('active cookie[' + i18nCookiename + ']='
			+ getCookie(i18nCookiename))
	loadScript(language);
	loadProperties(language);
}

function loadScript(language) {
	var src = 'easyui/locale' + '/easyui-lang-' + language.replace("-", "_")
			+ '.js';// when login in China the language=zh-CN
	$.getScript(src, function(response, status) {
		console.log("Script loaded and executed.src=" + src + ",status="
				+ status);
	});
}
function setLayoutTitle(title) {
	try {
		if ($.i18n.prop(title + '_title')) {
			...
			$('#zkweb_body').layout('panel', title).panel('setTitle',
					$.i18n.prop(title + '_title'));
		} else {
			console.log(title + '_title' + " not found in " + i18nSettings.name
					+ "_" + i18nSettings.language + ".propertities");
		}
	} catch (e) {
		console.log(title + '_title' + " not found in " + i18nSettings.name
				+ "_" + i18nSettings.language + ".propertities");
			...
			function() {
				...
				try {
					if ($.i18n.prop(varname)) {
						console.log(varname + "-var=" + varname + ":"
								+ $.i18n.prop(varname) + ",old="
								+ $(this).window('options').title);
						$(this).window('setTitle', $.i18n.prop(varname));
					} else {
						console.log(varname + "-var=" + varname
								+ " not found in " + i18nSettings.name + "_"
								+ i18nSettings.language + ".propertities");
					}
				} catch (e) {
					console.log(varname + "-var=" + varname + " not found in "
							+ i18nSettings.name + "_" + i18nSettings.language
							+ ".propertities");
					console.log(e.stack);
				}
			});
			...
			.each(
					function() {
						for (var i = 0; i < $(this).tabs('tabs').length; i++) {
							var title = $(this).tabs('getTab', i).panel(
									'options').title;
							var tabid = i;
							var varname = $(this).attr("id") + '-' + tabid
									+ '-tab';
							try {
								if ($.i18n.prop(varname)) {
									console.log(varname + "-var=" + varname
								...
								console.log(varname + "-var=" + varname
										+ " not found in " + i18nSettings.name
										+ "_" + i18nSettings.language
										+ ".propertities");
								console.log(e.stack);
							}
						}
	...
	$(attrNameList).each(
			function() {
				...
				if (locale) {
					try {
						if ($.i18n.prop(locale)) {
							console.log(attrTrueName + "-var=" + locale + ":"
									+ $.i18n.prop(locale) + ",old(html)="
									+ $(this).html());
			...
			function() {
				...
				if (locale) {
					try {
						if ($.i18n.prop(locale)) {
							console.log(attrTrueName + "-var=" + locale + ":"
									+ $.i18n.prop(locale) + "," + valueName
									+ "(old,attr)=" + $(this).attr(valueName));
...
function localeMessager(type, titleKey, titleDefault, msgKey, msgDefault,
		textKey, textDefault, arg1, arg2) {
	var ok = $.i18n.prop('ok')
	if (!ok)
		ok = 'Ok'
